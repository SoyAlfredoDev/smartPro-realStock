...
export default function PrivacyPolicyView() {
  ...
  return (
    <section className="relative overflow-hidden bg-main py-12 sm:py-16 md:py-20">
      <div
        className="pointer-events-none absolute inset-0 bg-[radial-gradient(ellipse_at_top_right,_rgba(176,112,113,0.10),_transparent_50%)]"
        aria-hidden
      />
      <div
        className="pointer-events-none absolute inset-0 bg-[radial-gradient(ellipse_at_bottom_left,_rgba(176,112,113,0.06),_transparent_45%)]"
        aria-hidden
      />
        ...
        <div className="grid grid-cols-1 gap-10 lg:grid-cols-[minmax(220px,260px)_minmax(0,1fr)] lg:gap-14 xl:gap-16">
          ...
          >
            <nav
              ...
              className="rounded-2xl border border-border-default bg-surface/80 p-4 shadow-soft backdrop-blur-sm sm:p-5"
            >
              <p className="mb-3 text-xs font-bold uppercase tracking-[0.14em] text-terracota">
                En esta página
              </p>
              <ul className="space-y-1.5" role="list">
                {privacyPolicySections.map((section, index) => (
                  ...
                  >
                    <a
                      href={`#${section.id}`}
                      className="block rounded-lg px-3 py-2 text-sm leading-snug text-body transition-colors duration-200 hover:bg-brand/5 hover:text-terracota"
                    >
                      {section.title}
                    </a>
                  </motion.li>
          ...
          <div className="min-w-0">
            ...
            >
              ...
              <h1 className="text-3xl font-bold tracking-tight text-heading text-balance sm:text-4xl md:text-[2.5rem]">
                {privacyPolicyMeta.title}
              </h1>

              <div className="flex flex-wrap gap-2.5">
                <span className="inline-flex items-center rounded-full border border-brand/20 bg-brand/10 px-3 py-1 text-xs font-medium text-terracota">
                  {privacyPolicyMeta.lawReference}
                </span>
                <span className="inline-flex items-center rounded-full border border-border-default bg-surface px-3 py-1 text-xs font-medium text-muted">
              ...
              <p className="text-base leading-relaxed text-body sm:text-lg">
                ...
                información al navegar, suscribirse o utilizar nuestra plataforma de contenido.
              </p>
            ...
            <motion.footer
              className="mt-10 rounded-2xl border border-border-default bg-surface/80 p-6 shadow-soft backdrop-blur-sm sm:p-7"
              initial={reducedMotion ? false : { opacity: 0, y: 20 }}
            ...
            >
              <p className="text-sm leading-relaxed text-body sm:text-base">
                Si tiene dudas sobre el tratamiento de sus datos, utilice los canales
                publicados en {siteConfig.name}. También puede consultar el texto oficial de
                la norma en el{" "}
                <a
                  href="https://www.bcn.cl/leychile/navegar?idNorma=1203407"
                  target="_blank"
                  rel="noopener noreferrer"
                  className="inline-flex items-center gap-1 font-medium text-terracota transition-colors hover:underline"
                >
              ...
              <p className="mt-5 text-sm text-muted">
                <Link
                  href="/"
                  className="inline-flex items-center font-medium text-terracota transition-opacity hover:opacity-80"
                >
                  ← Volver al inicio
                </Link>
              </p>
